...
enum class NotificationChannels(displayName: String, channelId: String, description: String, importance: NotificationImportance) {
    BACKGROUND_SERVICE("Background Service", "background_service", "Required for Background Services", NotificationImportance.MIN),
    BAZAAR_TRACKER("Bazaar Tracker", "bazaar_tracker", "Bazaar Tracking Notifications", NotificationImportance.MAX),
    ...
    PRIORITY_LAUNCH(
        "Priority App Launch",
        "priority_launch",
        "Due too this App doing unconventional things which normally would be undesirable behaviour this is needed. While this Channel has Max priority it is recommended to deactivate the sound. It is activated by default so users which systems block the full screen intent at least get notified about it.",
