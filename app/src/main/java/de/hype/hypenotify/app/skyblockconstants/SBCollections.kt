...
interface SBCollections {
    ...
    val superCompactorLevel: Int
        get() = 160

    val minionID: String?

    enum class Farming(id: String?, minionID: String?, vararg tiers: Int) : SBCollections {
        ...
        override fun getTierCount(): Int {
            return tiers.size
        }

        override fun getTierWithCollection(amount: Int): Int {
            for (i in this.tiers.indices.reversed()) {
                if (tiers[i] >= amount) return i
            }
            return 0
        }

        override fun getCollectionForTier(tier: Int): Int {
            return tiers[min(tier, tiers.size - 1)]
        }

        override fun getMinionID(): String? {
            return minionID
        }
    }

    enum class Mining(id: String?, minionID: String?, vararg tiers: Int) : SBCollections {
    ...
    enum class Combat(id: String?, minionID: String?, vararg tiers: Int) : SBCollections {
    ...
    enum class Foraging(id: String?, minionID: String?, vararg tiers: Int) : SBCollections {
    ...
    enum class Fishing(id: String?, minionID: String?, vararg tiers: Int) : SBCollections {
    ...
    enum class Rift(id: String?, vararg tiers: Int) : SBCollections {
    ...
    companion object {
        fun values(): MutableSet<SBCollections> {
            val collections: MutableSet<SBCollections> = HashSet<SBCollections>()
            collections.addAll(Arrays.stream<Farming?>(Farming.entries.toTypedArray()).toList())
            collections.addAll(Arrays.stream<Foraging?>(Foraging.entries.toTypedArray()).toList())
            collections.addAll(Arrays.stream<Mining?>(Mining.entries.toTypedArray()).toList())
            collections.addAll(Arrays.stream<Fishing?>(Fishing.entries.toTypedArray()).toList())
            collections.addAll(Arrays.stream<Combat?>(Combat.entries.toTypedArray()).toList())
            return collections
        }
            ...
            for (value in values()) {
                if (value.displayName.equals(goalName, ignoreCase = true)) {
                    return value.javaClass.getName().replace("\\$[0-9]+".toRegex(), "").replace("$", ".")
                        .replace("de.hype.bbsentials.shared.constants.", "") + "." + value.toString()
                }
            }
            return null
        }
        ...
        val values: MutableSet<SBCollections> = values()
    }
}
